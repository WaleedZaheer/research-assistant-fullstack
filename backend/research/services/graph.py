...
class ResearchState(TypedDict):
    topic: str
    search_results: List[Dict]      # from Node 1: [{title, url, snippet}, ...]
    scraped_articles: List[Dict]    # from Node 2: [{url, content, source}, ...]
    summaries: List[Dict]           # from Node 3: [{url, summary}, ...]
    final_report: str               # from Node 4
...
def summarize_node(state: ResearchState) -> ResearchState:
    ...
    for article in state["scraped_articles"]:
        summary_text = summarize_article(state["topic"], article["content"])
        summaries.append({
            "url": article["url"],
